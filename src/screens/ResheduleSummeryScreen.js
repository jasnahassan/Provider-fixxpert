...
import { createBooking, fetchServiceTypeById, fetchAllAddresses, uploadBookingImages,rescheduleBooking } from '../redux/AuthSlice';

const BookingSummaryScreen = ({ navigation, route }) => {
    const [modalVisible, setModalVisible] = useState(false);
    const dispatch = useDispatch();
    const {
        addressId, selectedTab, selecteddate, selectedTime, description, serviceId, imagesToUpload,bookingId
    } = route.params;
    const { serviceDetails, loading, error } = useSelector((state) => state.auth);

    useEffect(() => {
        console.log(addressId, serviceId, imagesToUpload, selecteddate, selectedTime,bookingId, 'heree dataaa')
        if (serviceId) {
    ...
    const handleConfirmBooking = async () => {
        try {
            ...
            const data = await dispatch(rescheduleBooking({bookingId: bookingId,bookingData})).unwrap();
            console.log("✅ Booking response:", data);
            setModalVisible(true);

            if (imagesToUpload?.length > 0 || imagesToUpload != null) {
                try {
                ...
                <Modal transparent={true} visible={modalVisible} animationType="slide">
                    <View style={styles.modalContainer}>
                        <View style={styles.modalContent}>
                            ...
                            <Text style={styles.successText}>Booking Rescheduled !</Text>
                            <Text style={styles.modalDescription}>You have successfully re-scheduled your booking </Text>
